...
public interface AiUsageLogRepository extends JpaRepository<AiUsageLog, UUID> {
    ...
    @Query("SELECT COALESCE(SUM(l.estimatedCostUsd), 0) FROM AiUsageLog l")
    BigDecimal sumTotalEstimatedCost();

    @Query("SELECT COALESCE(AVG(l.latencyMs), 0) FROM AiUsageLog l WHERE l.status = 'SUCCESS'")
    double avgLatencyMs();

    @Query("SELECT COUNT(l) FROM AiUsageLog l WHERE l.status = 'FAILED'")
